...
// Helper macros for fetching TC info
...
typedef enum EPinmuxSelector
{
  ...
  PINMUX_ANALOG,           /* The pin is controlled by the associated signal of peripheral B. */
  PINMUX_SERCOM,           /* The pin is controlled by the associated signal of peripheral C. */
  PINMUX_SERCOM_ALT,       /* The pin is controlled by the associated signal of peripheral D. */
  PINMUX_TIMER,            /* The pin is controlled by the associated signal of peripheral E. */
  PINMUX_TIMER_ALT,        /* The pin is controlled by the associated signal of peripheral F. */
  PINMUX_COM,              /* The pin is controlled by the associated signal of peripheral G. */
  PINMUX_AC_CLK,           /* The pin is controlled by the associated signal of peripheral H. */
  PINMUX_DIGITAL,          /* The pin is controlled by PORT. */
  PINMUX_INPUT,            /* The pin is controlled by PORT and is an input. */
  PINMUX_INPUT_PULLUP,     /* The pin is controlled by PORT and is an input with internal pull-up resistor enabled. */
  PINMUX_OUTPUT,           /* The pin is controlled by PORT and is an output. */

  PINMUX_PWM=PINMUX_TIMER,
  PINMUX_PWM_ALT=PINMUX_TIMER_ALT,
} EPinmuxSelector;

// Pin attributes to be OR'd
...
// Pin Descriptor
// TODO: This can probably be packed more efficiently to save flash/ram
typedef struct _EPinDescriptor
{
  EPortType			port;
  uint32_t			pin;
  EPinmuxSelector	pinmuxSelect;
  uint32_t			pinAttribute;
  EAnalogChannel	adcChannel;
  EPWMChannel		pwmChannel;
  ETCChannel		tcChannel;
  EExtInterrupts 	extInt;
} EPinDescriptor;
